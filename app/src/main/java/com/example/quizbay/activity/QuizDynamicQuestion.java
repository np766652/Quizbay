...
public class QuizDynamicQuestion extends AppCompatActivity {
...
//                userScore.setSkippedQuestions(skipCount);
    ...
    public void setView(){
        ...
        questionString = sharedPreferences.getString("QuestionId"+1,"");
        ...
        else if (question.getQuestionType().equals("image"))
        {
            ...
            playerView.setVisibility(View.GONE);
            Glide.with(com.example.quizbay.activity.QuizDynamicQuestion.this)
        ...
        CountDownTimer countdownTimer= new CountDownTimer(timeRequired, 1000) {
    ...
    private void initializePlayer(String src) {
        Log.d("APi", "initializePlayer: "+src);
        player= getPlayer(com.example.quizbay.activity.QuizDynamicQuestion.this);
        playerView.setPlayer(player);
        MediaItem mediaItem = MediaItem.fromUri(src);
        player.setMediaItem(mediaItem);
        player.setPlayWhenReady(playWhenReady);
